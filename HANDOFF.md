...
## 0-0-b. 탱크 대작전(포트리스류) 확정 계획 (2026-07-24 밤, grill 완료 — Claude)

**기획만 확정, 구현은 추후.** 저작권: 게임 메커니즘은 저작권 대상 아님. 피할 것 = 포트리스2의 탱크 디자인 모방, "포트리스" 명칭(상표), 사운드. → 두들 스타일 오리지널 탱크 + 자체 이름 "탱크 대작전"(가칭).
...
| 렌더링 | CustomPainter (Flame 도입 안 함 — 턴제라 불필요. 러너 게임 때 재검토) |
...
| 캐릭터 | 커플 각자 탱크 색/모양 선택 (윷놀이 캐릭터 선택 패턴 재사용) |
| 타임아웃 | 없음. 기존 이탈 배너+restore. 정산은 정상 종료 시에만 |

### 구현 슬라이스 (추후)
1. `fortress-engine.js` + node --test: 지형 생성, 탄도 적분(중력+바람), 명중/스플래시/지형 파괴, 이동/연료/경사 검증, 서든데스, 무기 테이블
2. socket.js: `game:tank:new/move/fire/state` (볼링 패턴 — 서버 시뮬 결과에 궤적 좌표 포함) + 지갑 정산
3. Flutter: `tank_screen.dart` + 지형/탱크/궤적 CustomPainter, 드래그 조준, 폭발 컷씬, 판돈 선택 lobby UI
