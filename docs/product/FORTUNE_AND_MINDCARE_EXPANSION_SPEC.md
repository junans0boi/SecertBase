...
- Tarot lets each user or Couple choose one upright card from the versioned 22-card Major Arcana catalog. The first explicit choice is persisted by date and content version; before that choice the UI shows face-down card backs, and the same day has no re-selection or spreads. A birth profile is optional for Tarot.
