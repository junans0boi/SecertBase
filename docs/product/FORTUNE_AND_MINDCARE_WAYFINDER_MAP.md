...
- 타로는 개인별·Couple별로 22장 중 사용자가 직접 고른 하루 한 장을 카드 버전·날짜에 고정하고, 선택 전에는 카드 뒷면 목록을 보여준다. 같은 날 다시 고르기와 스프레드는 제공하지 않는다.
...
- [오늘의 운세 시각 구조와 사주·타로 정보 계약 #127](https://github.com/junans0boi/SecertBase/issues/127): 사주와 타로를 독립 페이지로 나누고, 각 페이지에서 개인 결과와 Couple 공유 결과를 구분한다. 실제 사주 계산은 사용자 제공 문서의 계산 기준을 버전화하고, 타로는 개인·Couple 범위별로 사용자가 고른 하루 한 장을 저장하며 선택 후 재선택하지 않는다.
...
- [타로 카탈로그·일일 저장 #133](https://github.com/junans0boi/SecertBase/issues/133) — 사용자 선택·날짜 고정·재선택 거부 코드·unit/REST fixture 완료; 인증 통합 fixture는 테스트 환경 제공 시 실행
- [타로 Flutter 화면 #134](https://github.com/junans0boi/SecertBase/issues/134) — 22장 카드 뒷면 선택 UI·선택 후 공개·API/widget 완료
