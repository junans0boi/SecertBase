...
const _categories = ['general', 'mbti', 'conflict', 'growth', 'gratitude', 'memory'];
const _categoryLabels = {'general': '일상', 'mbti': 'MBTI', 'conflict': '갈등', 'growth': '성장', 'gratitude': '감사', 'memory': '추억'};
const _categoryIcons = {'general': '📝', 'mbti': '🧠', 'conflict': '🌊', 'growth': '🌱', 'gratitude': '🙏', 'memory': '💌'};
...
class _ShelterScreenState extends State<ShelterScreen> with SingleTickerProviderStateMixin {
  ...
  Future<void> _loadJournals() async {
    ...
    try {
      final response = await http.get(Uri.parse('${_auth.baseUrl}/api/reflections?user_id=$uid'));
      final data = jsonDecode(response.body);
      if (response.statusCode == 200 && data['ok'] == true) {
        setState(() { _journals = data['reflections'] ?? []; });
      }
    } catch (_) {}
    finally { if (mounted) setState(() => _loadingJournal = false); }
  }
    ...
    try {
      ...
      if (response.statusCode == 200 && data['ok'] == true) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('마음속 이야기를 대피소에 담았어요 🍃')),
        );
      }
    } catch (_) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('저장 실패. 네트워크 상태를 확인해주세요.')));
    } finally {
        ...
        builder: (ctx, setDialogState) => AlertDialog(
                  ...
                  children: List.generate(_moods.length, (i) => GestureDetector(
                    ...
                    child: Container(
                      ...
                      decoration: BoxDecoration(
                        color: editMood == _moods[i] ? kMainRoseSoft : Colors.transparent,
                        borderRadius: BorderRadius.circular(10),
                        border: Border.all(color: editMood == _moods[i] ? kMainRose : Colors.transparent),
                      ),
          ...
          actions: [
            ...
            FilledButton(
              ...
              child: Text('저장', style: mainBody(color: Colors.white, weight: FontWeight.bold)),
            ),
          ],
        ),
      ),
  ...
  Future<void> _deleteJournal(int id) async {
    try {
      final response = await http.delete(Uri.parse('${_auth.baseUrl}/api/reflections/$id'));
      final data = jsonDecode(response.body);
      ...
      children: [
        ...
        Container(
          ...
          decoration: BoxDecoration(
            ...
            boxShadow: [BoxShadow(color: kMainRose.withValues(alpha: 0.08), blurRadius: 10)],
          ),
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
                ...
                child: ListView.builder(
                  ...
                  itemBuilder: (ctx, i) => GestureDetector(
                    ...
                    child: Container(
                      ...
                      decoration: BoxDecoration(
                        color: _selectedMood == _moods[i] ? kMainRoseSoft : Colors.transparent,
                        borderRadius: BorderRadius.circular(20),
                        border: Border.all(
                          color: _selectedMood == _moods[i] ? kMainRose : kMainRoseSoft,
                        ),
                      ),
                      child: Row(
                        mainAxisSize: MainAxisSize.min,
                        children: [
                          ...
                          Text(_moodLabels[i], style: mainBody(size: 11, color: _selectedMood == _moods[i] ? kMainRose : kMainMuted)),
                        ],
                      ),
                    ),
                  ),
                  ...
                  itemBuilder: (ctx, i) {
                    ...
                    return GestureDetector(
                      ...
                      child: Container(
                        ...
                        decoration: BoxDecoration(
                          color: _selectedCategory == cat ? kMainRose : Colors.white,
                          borderRadius: BorderRadius.circular(20),
                          border: Border.all(color: _selectedCategory == cat ? kMainRose : kMainRoseSoft),
                        ),
                        child: Text(
                          '${_categoryIcons[cat]} ${_categoryLabels[cat]}',
                          style: mainBody(size: 11, color: _selectedCategory == cat ? Colors.white : kMainInk),
                        ),
                      ),
                    );
                  },
              ...
              Align(
                ...
                child: FilledButton.icon(
                  ...
                  label: Text('비공개 저장', style: mainBody(size: 13, color: Colors.white, weight: FontWeight.bold)),
                  style: FilledButton.styleFrom(
                    backgroundColor: kMainRose,
                    shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10)),
                  ),
                  ...
                  ? Center(
                      child: Column(
                        ...
                        children: [
                          ...
                          const SizedBox(height: 12),
                          Text('마음속 이야기를 써보세요', style: mainBody(size: 14, color: kMainSub)),
                        ],
                      ),
                    )
                  : ListView.builder(
                        ...
                        return Container(
                          ...
                          child: Column(
                            ...
                            children: [
                              Row(
                                children: [
                                  Text(moodTag, style: const TextStyle(fontSize: 22)),
                                  ...
                                  const Spacer(),
                                  Text(date, style: mainBody(size: 11, color: kMainMuted)),
                                  const SizedBox(width: 8),
                                  GestureDetector(
                                    onTap: () => _editJournal(j),
                                    child: const Icon(Icons.edit_rounded, size: 16, color: kMainMuted),
                                  ),
                                  const SizedBox(width: 8),
                                  GestureDetector(
                                    onTap: () => showDialog(
                                      ...
                                      builder: (c) => AlertDialog(
                                        ...
                                        title: Text('삭제할까요?', style: mainTitle(size: 16)),
                                        actions: [
                                          TextButton(onPressed: () => Navigator.pop(c), child: Text('취소', style: mainBody(color: kMainMuted))),
                                          TextButton(
                                            onPressed: () { Navigator.pop(c); _deleteJournal(j['id']); },
                                            child: const Text('삭제', style: TextStyle(color: Colors.red)),
                                          ),
                                        ],
                                      ),
                                    ),
                                    child: const Icon(Icons.delete_rounded, size: 16, color: kMainMuted),
                                  ),
                                ],
                              ),
                              const SizedBox(height: 10),
                              Text(j['content'] ?? '', style: mainBody(size: 14, color: kMainInk)),
                            ],
                          ),
                        );
                      },
  ...
  Widget _buildGuidebookTab() {
    final guides = [
      _GuideItem(
        ...
        content: 'MBTI는 성격 유형일 뿐, 우열이 없어요. I와 E의 에너지 충전 방식이 다를 뿐입니다.\n\n'
      ...
      _GuideItem(
        ...
        content: '싸운 직후 억지로 화해하면 감정이 쌓입니다.\n\n'
      ...
      _GuideItem(
        icon: '⚖️',
        title: '수평적 관계를 만드는 법',
        content: '건강한 연애는 한쪽이 일방적으로 맞추거나 희생하지 않아요.\n\n'
      ...
      _GuideItem(
        ...
        content: '• 연락 주기: "적당한" 연락 주기는 커플마다 달라요. 초반에 명확히 이야기 나눠두세요.\n'
      ...
      _GuideItem(
        icon: '💪',
        title: '힘든 시기에 관계 지키기',
        content: '취업 걱정, 가족 문제, 건강 이슈… 힘든 시기에 연애가 흔들리는 건 자연스러운 일이에요.\n\n'
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      ...
      child: Container(
        ...
        child: Text(label, style: mainBody(size: 11, color: isSelected ? Colors.white : kMainInk)),
      ),
    );
  }
}
            ...
            child: Padding(
              ...
              child: Row(
                children: [
                  ...
                  Expanded(child: Text(widget.guide.title, style: mainBody(size: 15, color: kMainInk, weight: FontWeight.bold))),
                  Icon(_expanded ? Icons.keyboard_arrow_up_rounded : Icons.keyboard_arrow_down_rounded, color: kMainMuted),
                ],
              ),
            ),
          ),
          AnimatedSize(
            ...
            child: _expanded
                ? Padding(
                    ...
                    child: Text(widget.guide.content, style: mainBody(size: 13, color: kMainSub).copyWith(height: 1.65)),
                  )
